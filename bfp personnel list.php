...
if(isset($_POST['btnSave'])){
	...
	$insert_data = mysqli_query($conn, "INSERT INTO bfp_list (person,name,email,address,contact_no,position) 
	VALUES ('$person','$name','$email','$address','$contact_no','$position')");

	if($insert_data){
		header('Location:bfp personnel list.php?message=successadd');
	}else{
...
if(isset($_POST['btnUpdate'])){
	...
	mysqli_query($conn, "UPDATE bfp_list SET name= person='$person', name='$name', email='$email', address='$address', contact_no='$contact_no', position='$position' WHERE user_id=$id");
	header('location: bfp personnel list.php?message=successupdate');	
}
?>
...
if(isset($_GET['delete'])){
	$id = $_GET['delete'];
	mysqli_query($conn, "DELETE FROM bfp_list WHERE user_id=$id");
	header('location: bfp personnel list.php?message=successdelete');
}
?>
...
<body>
	<div class="container" style="margin-top: 15px">
		<a href="#addNewReportModal" class="btn btn-success btn-lg" data-toggle="modal"><span> Input record BFP Personnel of Binalonan</span></a>
	...
	<div class="container">
		...
		$(document).ready(function(){
			$("#myInput").on("keyup",function(){
				...
				$("#myTable tr").filter(function(){
					$(this).toggle($(this).text().toLowerCase().indexOf(value) > -1)
						...
						<h2>Beauro of Fire <b>Protection of Binalonan</b></h2>
					</div>
				</div>
			</div>
			<table class="table table-striped table-hover">
				...
				<?php
					$select_data = mysqli_query($conn, "SELECT * FROM bfp_list");
				...
				<tbody>
					<tr>
						...
						<td>
							...
							<a href="bfp personnel list.php?delete=<?php echo $user ['user_id']?>"class="delete" onclick="return confirm ('Are you sure you want to delete this user?')" name="btnDelete"><i class="material-icons" data-toggle="tooltip" title="Delete">&#xE872;</i
							></a>
						</td>
					</tr>
				</tbody>
