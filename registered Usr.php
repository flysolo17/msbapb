...
if(isset($_POST['btnSave'])){
	...
	$insert_data = mysqli_query($conn, "INSERT INTO registered_users (person,full_name,email,contact_no,position) 
	VALUES ('$person','$full_name','$email','$contact_no','$position')");

	if($insert_data){
		header('Location: registered Usr.php?message=successadd');
	}else{
...
if(isset($_POST['btnUpdate'])){
	...
	mysqli_query($conn, "UPDATE registered_users SET person='$person', full_name='$full_name', email='$email', contact_no='$contact_no', position='$position' WHERE user_id=$id");
	header('location: registered Usr.php?message=successupdate');
}
?>
...
if(isset($_GET['delete'])){
	$id = $_GET['delete'];
	mysqli_query($conn, "DELETE FROM registered_users WHERE user_id=$id");
	header('location: registered Usr.php?message=successdelete');
}
?>
...
<body>
	<div class="container" style="margin-top: 15px">
		<a href="#addUserModal" class="btn btn-success btn-lg" data-toggle="modal"><span>Add New User</span></a>
	...
	<div class="container">
		...
		$(document).ready(function(){
			$("#myInput").on("keyup",function(){
				...
				$("#myTable tr").filter(function(){
					$(this).toggle($(this).text().toLowerCase().indexOf(value) > -1)
						...
						<h2>Manage<b> Registered Users</b></h2>
					</div>
				</div>
			</div>
			<table class="table table-striped table-hover">
				...
				<?php
					$select_data = mysqli_query($conn, "SELECT * FROM registered_users");
				...
				<tbody id="myTable">
					<tr>
						...
						<td>
							...
							<a href="registered Usr.php?delete=<?php echo $user ['user_id']?>"class="delete" onclick="return confirm ('Are you sure you want to delete this user?')" name="btnDelete"><i class="material-icons" data-toggle="tooltip" title="Delete">&#xE872;</i
							></a>
						</td>
					</tr>
				</tbody>
			...
			<div class="modal-content">
				<form method="POST" action="#">
					<div class="modal-header">
						<h4 class="modal-title">Add Registered User</h4>
					...
					<div class="form-group">
						...
						<div class="form-group">
							<label>Name</label>
							<input type="text" full_name="full_name" class="form-control" required>
						</div>
						<div class="form-group">
							<label>Email</label>
	...
	<div id="editUserModal" class="modal fade">
		<div class="modal-dialog">
			<div class="modal-content">
				<form method="POST" action="#">
					<div class="modal-header">
						<h4 class="modal-title">Edit Registered User</h4>
						<button type="button" class="close" data-dismiss="modal" aria-hidden="true">Close</
						button>
						</div>
					<div class="modal-body">
