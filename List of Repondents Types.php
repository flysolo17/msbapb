...
if(isset($_POST['btnSave'])){
	...
	$insert_data = mysqli_query($conn, "INSERT INTO list_of_respondent_types (person,name,date_created,status) 
	VALUES ('$person','$name','$date_created','$status')");

	if($insert_data){
		header('list of Respondent Types.php?message=successadd');
	}else{
...
if(isset($_POST['btnUpdate'])){
	...
	mysqli_query($conn, "UPDATE list_of_respondent_types SET  person='$person', name='$name', date_created='$date_created', status='$status' WHERE user_id=$id");
	header('location: List of Repondents Types.php?message=successupdate');
}
?>
...
if(isset($_GET['delete'])){
	$id = $_GET['delete'];
	mysqli_query($conn, "DELETE FROM list_of_respondent_types WHERE user_id=$id");
	header('location: List of Repondents Types.php?message=successdelete');
}
?>
	...
	<div class="container">
		...
		<div class="table-wrapper">
			...
			<table class="table table-striped table-hover">
				...
				<?php
					$select_data = mysqli_query($conn, "SELECT * FROM list_of_respondent_types");
				...
				<tbody id="myTable">
					<tr>
						...
						<td>
							...
							<a href="List of Repondents Types.php?delete=<?php echo $user ['user_id']?>"class="delete" onclick="return confirm ('Are you sure you want to delete this user?')" name="btnDelete"><i class="material-icons" data-toggle="tooltip" title="Delete">&#xE872;</i
							></a>
						</td>
					</tr>
				</tbody>
