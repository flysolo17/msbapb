...
if(isset($_POST['btnSave'])){
	...
	$insert_data = mysqli_query($conn, "INSERT INTO pnp_list (person,name,email,address,contact_no,position) 
	VALUES ('$person','$name','$email','$address','$contact_no','$position')");

	if($insert_data){
		header('Location:pnp personnel list.php?message=successadd');
	}else{
...
if(isset($_POST['btnUpdate'])){
	...
	mysqli_query($conn, "UPDATE pnp_list SET name= person='$person', name='$name', email='$email', address='$address', contact_no='$contact_no', position='$position' WHERE user_id=$id");
	header('location: pnp personnel list.php?message=successupdate');	
}
?>
...
if(isset($_GET['delete'])){
	$id = $_GET['delete'];
	mysqli_query($conn, "DELETE FROM pnp_list WHERE user_id=$id");
	header('location: pnp personnel list.php?message=successdelete');
}
?>
...
<body>
	<div class="container" style="margin-top: 15px">
		<a href="#addNewReportModal" class="btn btn-success btn-lg" data-toggle="modal"><span> Input record PNP Personnel of Binalonan</span></a>
	...
	<div class="container">
			...
			<div class="table-title">
				<div class="row">
					...
					<div class="form-group">
						<input type="text" id="myInput" placeholder="Search Personnel..." class="form-control">
					</div>
					<script>
		$(document).ready(function(){
			$("#myInput").on("keyup",function(){
				...
				$("#myTable tr").filter(function(){
					$(this).toggle($(this).text().toLowerCase().indexOf(value) > -1)
						...
						<h2>Philippine National <b>Police of Binalonan</b></h2>
					</div>
				</div>
			</div>
			<table class="table table-striped table-hover">
				...
				<?php
					$select_data = mysqli_query($conn, "SELECT * FROM pnp_list");
				...
				<tbody>
					<tr>
						...
						<td>
							...
							<a href="pnp personnel list.php?delete=<?php echo $user ['user_id']?>"class="delete" onclick="return confirm ('Are you sure you want to delete this user?')" name="btnDelete"><i class="material-icons" data-toggle="tooltip" title="Delete">&#xE872;</i
							></a>
						</td>
					</tr>
				</tbody>
	...
	<div id="editReportModal" class="modal fade">
		<div class="modal-dialog">
			<div class="modal-content">
				<form method="POST" action="#">
					<div class="modal-header">
						<h4 class="modal-title">Edit Personnels Record</h4>
