...
if(isset($_POST['btnSave'])){
	...
	$insert_data = mysqli_query($conn, "INSERT INTO pnp_duty (full_name,email,address,contact_no,position,date_and_time) 
	VALUES ('$full_name','$email','$address','$contact_no','$position','$date_and_time')");

	if($insert_data){
		header('Location:PNP ON DUTY.php?message=successadd');
	}else{
...
if(isset($_POST['btnUpdate'])){
	...
	mysqli_query($conn, "UPDATE pnp_duty SET name= full_name='$full_name', email='$email', address='$address', contact_no='$contact_no', position='$position', date_and_time='$date_and_time' WHERE user_id=$id");
	header('location: PNP ON DUTY.php?message=successupdate');	
}
?>
...
if(isset($_GET['delete'])){
	$id = $_GET['delete'];
	mysqli_query($conn, "DELETE FROM pnp_duty WHERE user_id=$id");
	header('location: PNP ON DUTY.php?message=successdelete');
}
?>
	...
	<div class="container">
		...
		<div class="table-wrapper">
			...
			<table class="table table-striped table-hover">
				...
				<?php
					$select_data = mysqli_query($conn, "SELECT * FROM pnp_duty");
                    ...
                    <tbody id="myTable">
						...
						<td>
                         ...
                         <a href="#"class="edit" id="btnEditmodal"name="btnEditmodal" data-toggle="modal"
							...
							<a href="PNP ON DUTY.php?delete=<?php echo $user ['user_id']?>"class="delete" onclick="return confirm ('Are you sure you want to delete this user?')" name="btnDelete"><i class="material-icons" data-toggle="tooltip" title="Delete">&#xE872;</i
							></a>
						</td>
					</tr>
				</tbody>
